...
if not exist ".venv\Scripts\python.exe" (
  echo [0/6] Creating an isolated BACH Studio Python environment...
...
echo [1/6] Checking BACH Studio source integrity...
"%BACH_PY%" code\smoke_test.py
if errorlevel 1 goto :setup_failed

echo.
echo [2/6] Updating pip in the isolated environment...
"%BACH_PY%" -m pip install --upgrade pip
if errorlevel 1 goto :setup_failed

echo.
echo [3/6] Installing and verifying CUDA-enabled PyTorch...
"%BACH_PY%" code\setup_cuda.py
if errorlevel 1 goto :setup_failed

echo.
echo [4/6] Installing remaining BACH Studio Python dependencies...
"%BACH_PY%" -m pip install -r code\requirements.txt
if errorlevel 1 goto :setup_failed

echo.
echo [5/6] Installing/verifying codec and generation models...
"%BACH_PY%" code\setup_runtime.py
if errorlevel 1 goto :setup_failed

echo.
echo [6/6] Starting BACH Studio...
cd /d "%~dp0code"
"%BACH_PY%" ui.py
if errorlevel 1 (
  echo.
...
echo source or generation environment did not pass setup/verification.
