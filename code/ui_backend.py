...
def compose_style(genres, instruments, moods, gender, timbres, extra=None) -> str:
    parts = []
    for group in (_as_list(moods), _as_list([gender] if gender else []), _as_list(genres), _as_list(timbres), _as_list(instruments)):
        parts.extend(group)
...
def system_check() -> dict[str, Any]:
    ...
    add("Inference launcher", INFER_SCRIPT.exists(), str(INFER_SCRIPT))
    ...
    cuda_detail, cuda_ok = "Unavailable", False
    try:
        import torch
    ...
    return {"ready": not missing, "checks": checks, "missing_count": len(missing)}
...
def system_check_markdown() -> str:
    ...
    lines.append("")
    lines.append("**Ready to generate.**" if report["ready"] else f"**Not ready:** {report['missing_count']} required check(s) are unresolved. The UI will not launch a generation job until these are fixed.")
    return "\n".join(lines)
...
def _new_job_dir() -> tuple[Path, Path]:
    ...
    input_dir, output_dir = INPUT_ROOT / job_id, OUTPUT_ROOT / job_id
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    return input_dir, output_dir


def build_command(genre_path, lyrics_path, output_dir, reference_mode, audio_reference, vocal_reference, instrumental_reference, prompt_start, prompt_end, seed, cuda_idx, run_n_segments, repetition_penalty, max_new_tokens, stage2_batch_size, keep_intermediate, disable_offload, rescale):
    cmd = [sys.executable, str(INFER_SCRIPT), "--genre_txt", str(genre_path), "--lyrics_txt", str(lyrics_path), "--output_dir", str(output_dir), "--seed", str(int(seed)), "--cuda_idx", str(int(cuda_idx)), "--run_n_segments", str(int(run_n_segments)), "--repetition_penalty", str(float(repetition_penalty)), "--max_new_tokens", str(int(max_new_tokens)), "--stage2_batch_size", str(int(stage2_batch_size))]
    ...
    if mode.startswith("single"):
        ...
        cmd += ["--use_audio_prompt", "--audio_prompt_path", path, "--prompt_start_time", str(float(prompt_start)), "--prompt_end_time", str(float(prompt_end))]
    elif mode.startswith("dual"):
        vocal, instrumental = _resolve_upload_path(vocal_reference), _resolve_upload_path(instrumental_reference)
        if not vocal or not instrumental:
            raise ValueError("Dual Reference mode requires both vocal and instrumental files.")
        cmd += ["--use_dual_tracks_prompt", "--vocal_track_prompt_path", vocal, "--instrumental_track_prompt_path", instrumental, "--prompt_start_time", str(float(prompt_start)), "--prompt_end_time", str(float(prompt_end))]
    if keep_intermediate: cmd.append("--keep_intermediate")
    if disable_offload: cmd.append("--disable_offload_model")
    if rescale: cmd.append("--rescale")
    return cmd


def _latest_matching(paths):
...
def discover_outputs(output_dir):
    ...
    return _latest_matching(root_audio) or _latest_matching(mixed_candidates), _latest_matching(vocal_candidates), _latest_matching(inst_candidates)


def generate_song(lyrics, genres, instruments, moods, gender, timbres, extra_style, reference_mode, audio_reference, vocal_reference, instrumental_reference, prompt_start, prompt_end, seed, cuda_idx, run_n_segments, repetition_penalty, max_new_tokens, stage2_batch_size, keep_intermediate, disable_offload, rescale):
    ...
    if not report["ready"]:
        ...
        raise RuntimeError("System preflight failed. Missing/unavailable:\n- " + "\n- ".join(missing))

    input_dir, output_dir = _new_job_dir()
    genre_path, lyrics_path = input_dir / "genre.txt", input_dir / "lyrics.txt"
    genre_path.write_text(style, encoding="utf-8")
    # Upstream infer.py caps its prompt slice one element too early. A private sentinel
    # makes the final real user section land inside that slice without modifying infer.py.
    lyrics_path.write_text(lyrics.strip() + "\n\n[ui_sentinel]\n\n", encoding="utf-8")

    cmd = build_command(genre_path, lyrics_path, output_dir, reference_mode, audio_reference, vocal_reference, instrumental_reference, prompt_start, prompt_end, seed, cuda_idx, run_n_segments, repetition_penalty, max_new_tokens, stage2_batch_size, keep_intermediate, disable_offload, rescale)
    process = subprocess.Popen(cmd, cwd=str(INFERENCE_DIR), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8", errors="replace", bufsize=1)
    lines = []
    ...
    return GenerationResult(mixed, vocal, instrumental, str(output_dir), log_text[-12000:], command_string)
